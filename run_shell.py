...
def get_input(about, items, shorter_period=False, first_choice=False):
    ...
    if about not in ["pollutants","n_days"]:
        message_to_user = "Choose a French "+about[:-1]+"."
        choices = [
            (e[0] if not(about=="stations") 
            else e[0][:e[0].index("#")])
            +" ("+str(e[1])+")" for e in items]
    elif about == "pollutants":
        message_to_user = "Choose an air pollutant."
        choices = [e[0]+" ("+str(e[1])+")" for e in items]
    ...
    # choices.
    if about not in ["regions","n_days"]:
        choices += [
            "\nReturn"+" ("+str(len(choices)+1)+")"]
...
def plot_variation(station, pollutant, values):
    ...
    # each of the 24 hours of the day are associated with an
...
def main():
    ...
    while not(process.done):
        ...
        process.next_step()
    # Send the given query parameters to the endpoint.
    response = requests.get(
        "http://127.0.0.1:8000",
        params=process.query_parameters,
        verify=False).json()
    # If an error occured, indicate the cause to the user.
    if response.status_code != 200:
        print("\n"+response["detail"])
    # If not, generate the expected data visualization using
    # the values provided by the response.
    else:
        plot_variation(
            process.query_parameters["station_name"],
            process.query_parameters["p"],
            response["values"])
